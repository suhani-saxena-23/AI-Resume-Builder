...
export const enhanceProfessionalSummary = async (req, res) => {
    try {
       ...
       const response = await ai.chat.completions.create({
            ...
            messages: [
                { role: "system", content: "You are an expert in reYou are an expert in resume writing. Your task is to enhance the professional summary of a resume. The summary should be 1-2 sentences also highlighting key skills, experience, and career objectives. Make it compelling and ATS-friendly, and only return text no options or anything else." },
                {
                    role: "user",
                    content: userContent,
                },
...
export const uploadResume= async (req, res) => {
    try {
        ...
        console.log('[AI Upload] received uploadResume request, title:', title, 'resumeTextLength:', resumeText ? resumeText.length : 0);
